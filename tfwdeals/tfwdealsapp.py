...
class TfwDealsApp(object):
    ...
    def generate_deals_page(self, links):
        ...
        for link in links:
            if u'Banner' not in link[ 'link-type' ]:
                if link[ 'coupon-code' ] and link[ 'coupon-code' ].find(
                    'no code') == -1:
        ...
        for link in text_links:
            link_code_html = link[
                'link-code-html' ]  # .encode('ascii', 'ignore')
        ...
        if coupon_links:
            ...
            # fixme - this is a really janky way to find the percent discounted
        ...
        my_path = os.path.dirname(
            __file__)  # path to where this script is located
    ...
    def html_changed(self, pathname):
        ...
        changed = True
        pn, fn = os.path.split(
            pathname)  # files are stored where script resides
    ...
    def run(self):
        ...
        for index, (brand_key, brand_dict) in enumerate(
            self._tfwbrands.items()):  # pylint: disable=unused-variable,line-too-long
            ...
            if self.html_changed(html_path):
                ...
                if not update_email_message:
                    update_email_message = (
                                               "The following pages were updated today"
                                               " on %s\n") % self._spm_website_name
                update_email_message = update_email_message + x + '\n'

            else:
                print(
                    '%s - No changes %s' % (html_path, 'smartphonematters.com'))

        self.html_file_cleanup(os.path.dirname(__file__),
                               self._tfwbrands.keys())
